...
Activity 1  [Moderate]
Modify the general workflow presented in the video to fit an SDM using presence only data for the Plains zebra (**Equus quagga**). The data is available at the following asset:  `users/ramirocrego84/`

The model needs to include the following:
- Grain size: 10000 m
- Predictor variables: elevation, slope, temperature seasonality, max temperature of warmest month, min temperature of coldest month, mean annual precipitation, mean NDVI from MODIS for the last 10 years (MODIS/006/MOD13Q1) and human modification index (CSP/HM/GlobalHumanModification)
