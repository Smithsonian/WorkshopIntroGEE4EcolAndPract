...
We first cover the basics of using matchine learning to create a simple species distribution model. 
We then cover more specifics of the SDM workflow for importing data and setting the main
...
2.  Generate pseudo-absences within a specified distance from presence
...
3.  Limit the area to select pseudo-absences by implementing an
...
creating the mask: `var mask2 = Clresult.select(['cluster']).eq(clustID)`.
...
Let's start running a very simple SDM using Random Forest algorithm. To do this, we will use a dataset on Plains zebra (**Equus quagga**) observations I downloaded from GBIF. 
...
Now we can load the 1593 zebra observations across Africa. We will not worry about data quality for now as we just want to demonstrate how SDMs work.
...
Next, we need to define a gran size to work with, the spatial resolution of the analysis. We will use a coarse resolution to work at the continental level of 10 x 10 km (10000 x 10000 m)
...
Next, we will load a set of predictor variables. We will use an elevation model and the 19 bioclimatic variables available on GEE. We can display annual mean temperature as an example.
...
So far we have the presence data and the predictor variables. To fit a model we also need pseudo-absences, so points that provide information on areas where we do not have zebra locations. This can be a complex and crucial step and we will discuss more in the next example, but for now, we will just create random points across the entire continent. Note, we need to add a property to the data indicating which ones are presence with a 1 and which ones are pseudo-absences with a 0.
...
Despite the simplicity of this model, we can see that the predicted distribution is not too far off from the actual distribution of Planis zebras.
...
<figcaption aria-hidden="true">Figure 10. Visualization of predicted
...
<figcaption aria-hidden="true">Figure 11. Potential distribution of
...
The idea of this assignment is that you can practice modifying the general workflow we presented here for implementing species distribution models in GEE. 

Activity 1  [Moderate]
Modify the general workflow presented in the video to fit an SDM using presence only data for the Plains zebra (**Equus quagga**). The data is available at the following asset:  `users/ramirocrego84/Perissodactyla`
Note that this includes other species, so you need to filter the data to retain only the Plains zebra data. Note you may also need to do some extra filters for data cleaning. 

The model needs to include the following:
- Grain size: 10000 m

- Study area: decide whether you want to use the default settings or define your own.

- Predictor variables: elevation, slope, temperature seasonality, max temperature of warmest month, min temperature of coldest month, mean annual precipitation, mean NDVI from MODIS for the last 10 years (MODIS/061/MOD13A2) and human modification index (CSP/HM/GlobalHumanModification)

- Create pseudo-absences randomly within the continent of Africa boundary (option 1 from the tutorial)

- Define your own block size
